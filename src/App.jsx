...
const App = () => {
  ...
  const [processes, setProcesses] = useState([
    ...
    { id: 5, arrivalTime: 4, burstTime: 4, priority: 1 }
  ]);
  const [algorithm, setAlgorithm] = useState('FCFS');
  const [timeQuantum, setTimeQuantum] = useState(2);
  const [results, setResults] = useState(null);
  const [showComparison, setShowComparison] = useState(false);
  const [animatingProcess, setAnimatingProcess] = useState(false);
  ...
  const facts = [
    "FCFS is the simplest CPU scheduling algorithm but can cause the 'convoy effect'",
    "SJF minimizes average waiting time but may cause starvation of longer processes",
    "Round Robin is designed for time-sharing systems and provides fairness",
    "Priority scheduling can lead to indefinite blocking or 'starvation'",
    "Preemptive algorithms allow the OS to interrupt running processes",
    "The average waiting time is a crucial metric in CPU scheduling performance",
    "Modern OS use multilevel queue scheduling for different process types",
    "CPU scheduling can improve overall system throughput by up to 50%",
    "Aging is a technique used to prevent starvation in priority scheduling",
    "SJF can lead to the 'starvation problem' for long processes"
  ];

  const quizQuestions = [
    {
      question: "Which algorithm is best for minimizing average waiting time?",
      options: ["FCFS", "SJF", "Round Robin", "Priority"],
      correct: 1,
      explanation: "SJF (Shortest Job First) minimizes average waiting time by executing shortest processes first.",
      difficulty: "easy"
    },
    {
      question: "Which algorithm can cause starvation of long processes?",
      options: ["FCFS", "SJF", "Round Robin", "SRTF"],
      correct: 1,
      explanation: "SJF always picks the shortest process, potentially ignoring longer processes indefinitely.",
      difficulty: "medium"
    },
    {
      question: "What does FCFS stand for?",
    ...
    {
      question: "Which provides better response time for interactive systems?",
      options: ["FCFS", "SJF", "Round Robin", "Priority"],
      correct: 2,
      explanation: "Round Robin provides better response time by giving each process a time slice.",
      difficulty: "medium"
    },
    {
      question: "What is the main goal of CPU scheduling?",
      options: ["Increase memory", "Maximize CPU utilization", "Delete processes faster", "Compress data"],
      correct: 1,
      explanation: "CPU scheduling aims to maximize CPU utilization and minimize waiting time.",
      difficulty: "easy"
    },
    {
      question: "Which algorithm is best for preventing starvation?",
    ...
    {
      question: "What's the difference between SJF and SRTF?",
      options: ["No difference", "SRTF is preemptive", "SJF is faster", "SRTF uses more memory"],
      correct: 1,
      explanation: "SRTF (Shortest Remaining Time First) is the preemptive version of SJF.",
      difficulty: "hard"
    },
    {
      question: "What happens when time quantum is too large in Round Robin?",
      options: ["Better performance", "Behaves like FCFS", "Processes starve", "System crashes"],
      correct: 1,
      explanation: "If time quantum is too large, Round Robin behaves like FCFS.",
      difficulty: "easy"
    }
  ];
  ...
  const animateProcessExecution = async () => {
    if (!results) return;
    
    setAnimatingProcess(true);
    setCurrentTime(0);
    
    for (let i = 0; i < results.timeline.length; i++) {
      await new Promise(resolve => setTimeout(resolve, 500 / animationSpeed));
      setCurrentTime(results.timeline[i].time);
    }
    
    setAnimatingProcess(false);
  };
  ...
  const getAIRecommendation = () => {
    ...
    // Calculate pattern characteristics
    const avgBurst = burstTimes.reduce((sum, t) => sum + t, 0) / burstTimes.length;
    const maxBurst = Math.max(...burstTimes);
    const minBurst = Math.min(...burstTimes);
    const burstVariance = burstTimes.reduce((sum, t) => sum + Math.pow(t - avgBurst, 2), 0) / burstTimes.length;
    
    // Check if burst times are similar
    const isSimilarBurst = burstVariance < 4;
    
    // Check if processes arrive together
    const maxArrival = Math.max(...arrivalTimes);
    const minArrival = Math.min(...arrivalTimes);
    const arrivalSpread = maxArrival - minArrival;
    const arriveTogether = arrivalSpread <= 2;
    
    // Check for short vs long processes
    const shortProcesses = processes.filter(p => p.burstTime <= avgBurst).length;
    const longProcesses = processes.length - shortProcesses;
    
    // Run all algorithms to get actual performance
    const algoPerformance = compareAllAlgorithms();
    const bestAlgo = algoPerformance.reduce((best, current) => 
      current.avgWaitingTime < best.avgWaitingTime ? current : best
    );
    
    const worstAlgo = algoPerformance.reduce((worst, current) => 
      current.avgWaitingTime > worst.avgWaitingTime ? current : worst
    );
    
    let recommendation = {
      best: bestAlgo.algorithm,
      estimatedWaitTime: bestAlgo.avgWaitingTime,
      reasons: [],
      avoid: [],
      pattern: '',
      confidence: 0,
    ...
    // Determine pattern and reasoning
    if (isSimilarBurst && arriveTogether) {
      recommendation.pattern = '⚖️ Uniform Workload Pattern';
      recommendation.reasons.push('Processes have similar burst times and arrive together');
      recommendation.reasons.push('FCFS performs well when processes are uniform');
      recommendation.confidence = 90;
    } else if (burstVariance > 10) {
      recommendation.pattern = '📊 Mixed Workload Pattern';
      recommendation.reasons.push(`High variance in burst times (σ²: ${burstVariance.toFixed(1)})`);
      recommendation.reasons.push('SJF/SRTF excel at minimizing wait time for varied workloads');
      recommendation.confidence = 95;
    } else if (shortProcesses > longProcesses * 2) {
      recommendation.pattern = '⚡ Short Process Dominant';
      recommendation.reasons.push(`${shortProcesses} short processes vs ${longProcesses} long processes`);
      recommendation.reasons.push('SJF provides best average waiting time for short processes');
      recommendation.confidence = 85;
    } else if (processes.length >= 5) {
      recommendation.pattern = '🔄 Interactive System Pattern';
      recommendation.reasons.push(`${processes.length} processes requiring fair CPU time`);
      recommendation.reasons.push('Round Robin provides fairness and good response time');
      recommendation.confidence = 80;
    } else {
      recommendation.pattern = '🎯 General Purpose Pattern';
      recommendation.reasons.push(`Standard workload with ${processes.length} processes`);
      recommendation.reasons.push('Multiple algorithms can work efficiently');
      recommendation.confidence = 75;
    }
    
    // Add performance comparison
    const improvement = ((worstAlgo.avgWaitingTime - bestAlgo.avgWaitingTime) / worstAlgo.avgWaitingTime * 100).toFixed(1);
    recommendation.reasons.push(`${bestAlgo.algorithm} reduces wait time by ${improvement}% vs ${worstAlgo.algorithm}`);
    
    // Algorithms to avoid
    if (worstAlgo.avgWaitingTime > bestAlgo.avgWaitingTime * 1.5) {
      recommendation.avoid.push(`❌ ${worstAlgo.algorithm} has ${worstAlgo.avgWaitingTime.toFixed(2)}ms avg wait (${((worstAlgo.avgWaitingTime / bestAlgo.avgWaitingTime - 1) * 100).toFixed(1)}% slower)`);
    }
    
    // FCFS specific warning
    const fcfsPerf = algoPerformance.find(a => a.algorithm === 'FCFS');
    if (bestAlgo.algorithm !== 'FCFS' && fcfsPerf.avgWaitingTime > bestAlgo.avgWaitingTime * 1.3) {
      recommendation.avoid.push(`⚠️ FCFS inefficient for this workload (${fcfsPerf.avgWaitingTime.toFixed(2)}ms avg wait)`);
    }
  ...
  const compareAllAlgorithms = () => {
    ...
    const comparisonResults = algorithms.map(algo => {
      const result = calculateSchedulingForAlgorithm(algo);
      return {
        algorithm: algo,
  ...
  const calculateSchedulingForAlgorithm = (algo) => {
    let sequence = [];
    let timeline = [];
    let currentTime = 0;
    let completed = [];
    ...
    switch(algo) {
      case 'FCFS':
        ...
        processesCopy.forEach(p => {
          ...
          timeline.push({ processId: p.id, time: currentTime, type: 'start' });
          currentTime += p.burstTime;
          p.completionTime = currentTime;
          p.turnaroundTime = p.completionTime - p.arrivalTime;
          p.waitingTime = p.turnaroundTime - p.burstTime;
          timeline.push({ processId: p.id, time: currentTime, type: 'end' });
          sequence.push(p);
          waitingTimes.push(p.waitingTime);
          turnaroundTimes.push(p.turnaroundTime);
        });
        break;

      case 'SJF':
        let remainingProcs = [...processesCopy];
        while (remainingProcs.length > 0) {
          let available = remainingProcs.filter(p => p.arrivalTime <= currentTime);
          if (available.length === 0) {
            currentTime = Math.min(...remainingProcs.map(p => p.arrivalTime));
            available = remainingProcs.filter(p => p.arrivalTime <= currentTime);
          }
          let shortest = available.reduce((prev, curr) => 
            curr.burstTime < prev.burstTime ? curr : prev
          );
          shortest.startTime = currentTime;
          timeline.push({ processId: shortest.id, time: currentTime, type: 'start' });
          currentTime += shortest.burstTime;
          shortest.completionTime = currentTime;
          shortest.turnaroundTime = shortest.completionTime - shortest.arrivalTime;
          shortest.waitingTime = shortest.turnaroundTime - shortest.burstTime;
          timeline.push({ processId: shortest.id, time: currentTime, type: 'end' });
          sequence.push(shortest);
          waitingTimes.push(shortest.waitingTime);
          turnaroundTimes.push(shortest.turnaroundTime);
          remainingProcs = remainingProcs.filter(p => p.id !== shortest.id);
        }
        break;

      case 'SRTF':
        let remaining = processesCopy.map(p => ({...p}));
        let executing = null;
        let maxTime = Math.max(...processesCopy.map(p => p.arrivalTime + p.burstTime)) * 2;
        
        for (let time = 0; time <= maxTime && remaining.some(p => p.remainingTime > 0); time++) {
          ...
          if (available.length > 0) {
            ...
            if (!executing || executing.id !== shortest.id) {
              if (executing) {
                timeline.push({ processId: executing.id, time: time, type: 'preempt' });
              }
              timeline.push({ processId: shortest.id, time: time, type: 'start' });
              executing = shortest;
            }
            ...
            if (shortest.remainingTime === 0) {
              ...
              waitingTimes.push(shortest.waitingTime);
              turnaroundTimes.push(shortest.turnaroundTime);
              if (!sequence.find(p => p.id === shortest.id)) {
                sequence.push(shortest);
              }
              executing = null;
            }
      ...
      case 'Priority':
        let remainingPriority = [...processesCopy];
        while (remainingPriority.length > 0) {
          let available = remainingPriority.filter(p => p.arrivalTime <= currentTime);
          if (available.length === 0) {
            currentTime = Math.min(...remainingPriority.map(p => p.arrivalTime));
            available = remainingPriority.filter(p => p.arrivalTime <= currentTime);
          }
          let highest = available.reduce((prev, curr) => 
            curr.priority < prev.priority ? curr : prev
          );
          highest.startTime = currentTime;
          timeline.push({ processId: highest.id, time: currentTime, type: 'start' });
          ...
          timeline.push({ processId: highest.id, time: currentTime, type: 'end' });
          sequence.push(highest);
          waitingTimes.push(highest.waitingTime);
          turnaroundTimes.push(highest.turnaroundTime);
          remainingPriority = remainingPriority.filter(p => p.id !== highest.id);
        }
        break;

      case 'Priority-Preemptive':
        let remainingPP = processesCopy.map(p => ({...p}));
        let executingPP = null;
        let maxTimePP = Math.max(...processesCopy.map(p => p.arrivalTime + p.burstTime)) * 2;
        
        for (let time = 0; time <= maxTimePP && remainingPP.some(p => p.remainingTime > 0); time++) {
          let available = remainingPP.filter(p => p.arrivalTime <= time && p.remainingTime > 0);
          ...
          if (available.length > 0) {
            ...
            if (!executingPP || executingPP.id !== highest.id) {
              if (executingPP) {
                timeline.push({ processId: executingPP.id, time: time, type: 'preempt' });
              }
              timeline.push({ processId: highest.id, time: time, type: 'start' });
              executingPP = highest;
            }
            ...
            if (highest.remainingTime === 0) {
              ...
              waitingTimes.push(highest.waitingTime);
              turnaroundTimes.push(highest.turnaroundTime);
              if (!sequence.find(p => p.id === highest.id)) {
                sequence.push(highest);
              }
              executingPP = null;
            }
      ...
      case 'Round Robin':
        let queue = [];
        let remainingRR = processesCopy.map(p => ({...p}));
        let time = 0;
        let maxTimeRR = Math.max(...processesCopy.map(p => p.arrivalTime + p.burstTime)) * 3;
        
        for (time = 0; time <= maxTimeRR && remainingRR.some(p => p.remainingTime > 0); time++) {
          // Add newly arrived processes to queue
          remainingRR.filter(p => p.arrivalTime === time && p.remainingTime > 0).forEach(p => {
            if (!queue.includes(p)) queue.push(p);
          });
          
          if (queue.length > 0) {
            let current = queue.shift();
            timeline.push({ processId: current.id, time: time, type: 'start' });
            ...
            remainingRR.filter(p => p.arrivalTime <= time && p.arrivalTime > (time - executeTime + 1) && p.remainingTime > 0 && !queue.includes(p) && p.id !== current.id).forEach(p => {
              queue.push(p);
            });
            ...
            if (current.remainingTime === 0) {
              ...
              timeline.push({ processId: current.id, time: currentTime, type: 'end' });
              waitingTimes.push(current.waitingTime);
              turnaroundTimes.push(current.turnaroundTime);
              if (!sequence.find(p => p.id === current.id)) {
                sequence.push(current);
              }
            } else {
              timeline.push({ processId: current.id, time: currentTime, type: 'preempt' });
              queue.push(current);
            }
          }
        }
  ...
  const calculateScheduling = () => {
    const result = calculateSchedulingForAlgorithm(algorithm);
    setResults(result);
          ...
          <div className="max-w-6xl mx-auto">
            <div className="flex flex-col lg:flex-row gap-12 items-center">
              <div className="flex-1">
                ...
                <div className="space-y-6 text-lg text-gray-700 leading-relaxed">
                  <p className="border-l-4 border-purple-500 pl-6">
                    CPU scheduling is a <span className="font-semibold text-purple-600">fundamental operating system mechanism</span> that determines which process runs on the CPU and for how long. When multiple processes compete for CPU time, intelligent scheduling algorithms determine the optimal execution order.
                  </p>
                  <p className="border-l-4 border-blue-500 pl-6">
                    The primary objectives are to <span className="font-semibold text-blue-600">maximize CPU utilization</span>, minimize waiting time, and ensure fair allocation of CPU time among all processes in the system.
                  </p>
                  <p className="border-l-4 border-indigo-500 pl-6">
                    Different algorithms employ unique strategies, balancing between <span className="font-semibold text-indigo-600">response time</span>, <span className="font-semibold text-indigo-600">throughput</span>, and <span className="font-semibold text-indigo-600">fairness</span> to optimize system performance.
                  </p>
                </div>
              </div>
              ...
              <div className="lg:w-80">
                <div className="bg-gradient-to-br from-purple-500 to-pink-500 rounded-2xl p-8 text-white shadow-2xl transform hover:scale-105 transition-all duration-300">
                  ...
                  <h3 className="text-2xl font-bold mb-3">Test Your Knowledge!</h3>
                  <p className="text-purple-100 mb-6 leading-relaxed">
                    Think you understand CPU scheduling? Challenge yourself with our interactive quiz and prove your expertise!
                  </p>
                  <button
                    onClick={startGame}
                    className="w-full px-6 py-3 bg-white text-purple-600 rounded-xl font-bold text-lg hover:bg-purple-50 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1">
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
              <AlgorithmCard
                icon={Clock}
                title="FCFS"
                description="First-Come, First-Served processes requests in the order they arrive. Simple but can lead to poor waiting times with long processes first."
                delay="0.1s"
              />
              <AlgorithmCard
                icon={Search}
                title="SJF"
                description="Shortest Job First selects the process with shortest burst time. Minimizes average waiting time but may cause starvation of long processes."
                delay="0.2s"
              />
              <AlgorithmCard
                icon={Scan}
                title="SRTF"
                description="Shortest Remaining Time First is preemptive SJF. Interrupts running process if a shorter one arrives, optimizing waiting time."
                delay="0.3s"
              />
              <AlgorithmCard
                icon={RefreshCw}
                title="Priority"
                description="Priority scheduling executes processes based on priority values. Higher priority processes run first but may cause starvation."
                delay="0.4s"
              />
              <AlgorithmCard
                icon={Eye}
                title="Priority-Preemptive"
                description="Preemptive priority scheduling can interrupt lower priority processes when higher priority ones arrive for better responsiveness."
                delay="0.5s"
              />
              <AlgorithmCard
                icon={RefreshCw}
                title="Round Robin"
                description="Time-sharing algorithm that gives each process a time quantum. Provides fairness and good response time for interactive systems."
                delay="0.6s"
              />
            </div>
            ...
            <div className="text-center py-16">
              <div className="max-w-2xl mx-auto bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-12 shadow-xl border border-gray-200">
                ...
                <p className="text-lg text-gray-600 mb-8 leading-relaxed">
                  Use our interactive simulator to visualize how each algorithm schedules
                  processes and compare their performance metrics.
                </p>
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  <button
                    onClick={() => setPage('simulator')}
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8 mb-8 animate-in fade-in slide-in-from-top duration-500">
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-6">
            ...
            {algorithm === 'Round Robin' && (
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Time Quantum</label>
                <input 
                  type="number"
                  value={timeQuantum}
          ...
          <div className="text-sm text-gray-600 mb-6 p-3 bg-blue-50 rounded-lg border border-blue-200">
            <p className="font-semibold mb-1">📊 Current Configuration:</p>
            <p>• Process Queue: {processes.length} processes | Algorithm: {algorithm} {algorithm === 'Round Robin' ? `| Time Quantum: ${timeQuantum}` : ''}</p>
          </div>
            ...
            <div id="ai-recommendation" className="mb-6 p-6 bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl border-2 border-purple-200 animate-in fade-in slide-in-from-top duration-500 shadow-lg">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg p-4 border-l-4 border-green-500 shadow-sm">
                  ...
                  <p className="text-sm text-gray-700">Estimated Avg Waiting Time: <span className="font-bold text-green-700">{aiRecommendation.estimatedWaitTime.toFixed(2)}ms</span></p>
                </div>
          ...
          <div className="space-y-4 mb-6">
            <div className="hidden md:grid md:grid-cols-4 gap-4 px-4 text-xs font-semibold text-gray-600">
              <div>PROCESS ID</div>
            ...
            {processes.map((p) => (
              <div key={p.id} className="grid grid-cols-1 md:grid-cols-4 gap-3 md:gap-4 p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-all duration-300 shadow-sm">
                <div>
                  <label className="block md:hidden text-xs font-semibold text-gray-600 mb-1">Process ID</label>
                  <input type="text" value={`P${p.id}`} disabled className="w-full px-3 py-2 bg-white border border-gray-300 rounded-lg text-gray-700 font-medium" />
                </div>
                <div>
                  <label className="block md:hidden text-xs font-semibold text-gray-600 mb-1">Arrival Time</label>
                  <input 
                    type="number"
                    value={p.arrivalTime}
                ...
                <div>
                  <label className="block md:hidden text-xs font-semibold text-gray-600 mb-1">Burst Time</label>
                  <input 
                    type="number"
                    value={p.burstTime}
                ...
                <div>
                  <label className="block md:hidden text-xs font-semibold text-gray-600 mb-1">Priority</label>
                  <div className="flex gap-2">
                    <input 
                      type="number"
                      value={p.priority}
          ...
          {results && (
            <div className="mt-6 p-4 bg-purple-50 rounded-lg border border-purple-200">
              <div className="flex items-center justify-between mb-2">
                <label className="text-sm font-semibold text-gray-700">Animation Speed: {animationSpeed}x</label>
                <button
                  onClick={animateProcessExecution}
                  disabled={animatingProcess}
                  className={`px-4 py-2 rounded-lg font-semibold transition-all duration-300 ${
                    animatingProcess 
                      ? 'bg-gray-300 text-gray-500 cursor-not-allowed' 
                      : 'bg-purple-600 hover:bg-purple-700 text-white'
                  }`}>
                  {animatingProcess ? '⏸️ Animating...' : '▶️ Animate Execution'}
                </button>
              </div>
              <input 
                type="range"
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8 mb-8 animate-in fade-in slide-in-from-top duration-500">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b-2 border-gray-200">
                    <th className="text-left py-3 px-4 font-semibold text-gray-700">Algorithm</th>
                    <th className="text-left py-3 px-4 font-semibold text-gray-700">Avg Waiting Time</th>
                    <th className="text-left py-3 px-4 font-semibold text-gray-700">Avg Turnaround Time</th>
                    <th className="text-left py-3 px-4 font-semibold text-gray-700">Performance</th>
                  </tr>
                </thead>
                <tbody>
                  {compareAllAlgorithms().map((result, idx) => {
                    const allResults = compareAllAlgorithms();
                    const minWait = Math.min(...allResults.map(r => r.avgWaitingTime));
                    const isBest = Math.abs(result.avgWaitingTime - minWait) < 0.01;
                    return (
                      <tr key={idx} className={`border-b border-gray-100 hover:bg-gray-50 transition-all duration-200 ${isBest ? 'bg-green-50' : ''}`}>
                        <td className="py-3 px-4 font-medium text-gray-800">{result.algorithm}</td>
                        <td className="py-3 px-4 text-gray-700">{result.avgWaitingTime.toFixed(2)}ms</td>
                        <td className="py-3 px-4 text-gray-700">{result.avgTurnaroundTime.toFixed(2)}ms</td>
                        <td className="py-3 px-4">
                          {isBest && <span className="px-3 py-1 bg-green-500 text-white rounded-full text-sm font-semibold">🏆 Best</span>}
                        </td>
                      </tr>
            ...
            <ResponsiveContainer width="100%" height={300} className="mt-8">
              <BarChart data={compareAllAlgorithms()}>
                ...
                <Tooltip />
                <Legend />
                <Bar dataKey="avgWaitingTime" fill="#8b5cf6" radius={[8, 8, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        )}
        ...
        {results && (
          <div id="results-section" className="space-y-8 animate-in fade-in slide-in-from-bottom duration-500">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="bg-white rounded-xl shadow-lg p-6 transform hover:scale-105 transition-all duration-300 border-l-4 border-purple-500">
                <h3 className="text-sm font-semibold text-gray-600 mb-2 uppercase tracking-wide">Average Waiting Time</h3>
                <p className="text-4xl font-bold text-purple-600">{results.avgWaitingTime.toFixed(2)}ms</p>
                <p className="text-xs text-gray-500 mt-1">time processes wait in queue</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-6 transform hover:scale-105 transition-all duration-300 border-l-4 border-pink-500">
                <h3 className="text-sm font-semibold text-gray-600 mb-2 uppercase tracking-wide">Average Turnaround Time</h3>
                <p className="text-4xl font-bold text-pink-600">{results.avgTurnaroundTime.toFixed(2)}ms</p>
                <p className="text-xs text-gray-500 mt-1">total time from arrival to completion</p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              <div className="flex items-center gap-2 mb-4 pb-3 border-b border-gray-200">
                <Activity className="w-6 h-6 text-indigo-600" />
                <h3 className="text-xl font-bold text-gray-800">Gantt Chart - Process Execution Timeline</h3>
              </div>
              <div className="overflow-x-auto">
                <div className="min-w-[600px] h-24 relative bg-gray-100 rounded-lg">
                  {results.timeline
                    .filter(t => t.type === 'start')
                    .map((start, idx) => {
                      const end = results.timeline.find(t => 
                        t.processId === start.processId && 
                        t.type === 'end' && 
                        t.time > start.time
                      );
                      if (!end) return null;
                      const duration = end.time - start.time;
                      const maxTime = Math.max(...results.timeline.map(t => t.time));
                      const startPercent = (start.time / maxTime) * 100;
                      const widthPercent = (duration / maxTime) * 100;
                      const colors = ['#9333ea', '#ec4899', '#6366f1', '#8b5cf6', '#d946ef', '#f97316'];
                      return (
                        <div
                          key={`${start.processId}-${idx}`}
                          className="absolute h-16 top-4 flex items-center justify-center text-white font-bold rounded shadow-md"
                          style={{
                            left: `${startPercent}%`,
                            width: `${widthPercent}%`,
                            backgroundColor: colors[start.processId % colors.length]
                          }}>
                          P{start.processId}
                          <div className="absolute -bottom-6 left-0 text-xs text-gray-600">{start.time}</div>
                          {idx === results.timeline.filter(t => t.type === 'start').length - 1 && (
                            <div className="absolute -bottom-6 right-0 text-xs text-gray-600">{end.time}</div>
                          )}
                        </div>
                      );
                    })}
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              <div className="flex items-center gap-2 mb-4 pb-3 border-b border-gray-200">
                <BarChart3 className="w-6 h-6 text-purple-600" />
                <h3 className="text-xl font-bold text-gray-800">Process Details</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b-2 border-gray-200">
                      <th className="text-left py-3 px-4 font-semibold text-gray-700">Process</th>
                      <th className="text-left py-3 px-4 font-semibold text-gray-700">Arrival</th>
                      <th className="text-left py-3 px-4 font-semibold text-gray-700">Burst</th>
                      <th className="text-left py-3 px-4 font-semibold text-gray-700">Completion</th>
                      <th className="text-left py-3 px-4 font-semibold text-gray-700">Waiting</th>
                      <th className="text-left py-3 px-4 font-semibold text-gray-700">Turnaround</th>
                    </tr>
                  </thead>
                  <tbody>
                    {results.processDetails.map((p, idx) => (
                      <tr key={idx} className="border-b border-gray-100 hover:bg-gray-50">
                        <td className="py-3 px-4 font-medium">P{p.id}</td>
                        <td className="py-3 px-4">{p.arrivalTime}</td>
                        <td className="py-3 px-4">{p.burstTime}</td>
                        <td className="py-3 px-4">{p.completionTime}</td>
                        <td className="py-3 px-4 text-purple-600 font-semibold">{p.waitingTime}</td>
                        <td className="py-3 px-4 text-pink-600 font-semibold">{p.turnaroundTime}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="grid lg:grid-cols-2 gap-8">
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                <div className="flex items-center gap-2 mb-4 pb-3 border-b border-gray-200">
                  <BarChart3 className="w-6 h-6 text-purple-600" />
                  <h3 className="text-xl font-bold text-gray-800">Waiting Times</h3>
                </div>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={results.processDetails.map(p => ({ name: `P${p.id}`, waiting: p.waitingTime }))}>
                    <XAxis dataKey="name" />
                    <YAxis />
                    <Tooltip />
                    <Legend />
                    <Bar dataKey="waiting" fill="#9333ea" radius={[8, 8, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                <div className="flex items-center gap-2 mb-4 pb-3 border-b border-gray-200">
                  <PieChart className="w-6 h-6 text-pink-600" />
                  <h3 className="text-xl font-bold text-gray-800">Turnaround Times</h3>
                </div>
                <ResponsiveContainer width="100%" height={300}>
                  <RPieChart>
                    <Pie
                      data={results.processDetails.map(p => ({ name: `P${p.id}`, value: p.turnaroundTime }))}
                      cx="50%"
                      cy="50%"
                      labelLine={false}
                      ...
                      fill="#8884d8"
                      dataKey="value">
                      {results.processDetails.map((entry, index) => (
                        <Cell key={`cell-${index}`} fill={['#9333ea', '#ec4899', '#6366f1', '#8b5cf6', '#d946ef', '#f97316'][index % 6]} />
                      ))}
                    </Pie>
                    <Tooltip />
